...
class TestKbFallbackToPexels(unittest.TestCase):
    """KB → Pexels 降级行为回归测试."""
    ...
    def test_kb_fallback_enabled_supplements_from_pexels(self):
        """
        source="knowledge_base" + kb_fallback_to_pexels=True:
        KB 素材时长不足时应自动从 Pexels 补充素材。
        回归保护 task.py L1542 修改。
        """
        config.app["pexels_api_keys"] = ["pexels-test-key"]
        config.app["material_directory"] = ""
        config.proxy.clear()

        kb_files = [
            "/tmp/test-kb-fallback/video-a.mp4",
            "/tmp/test-kb-fallback/video-b.mp4",
        ]

        pexels_item = material.MaterialInfo(
            provider="pexels",
            url="https://example.com/pexels-supplement.mp4",
            duration=5,
        )

        def fake_save(video_url, save_dir=""):
            return f"/tmp/test-kb-fallback/{video_url.rsplit('/', 1)[-1]}"

        class FakeClip:
            def __init__(self, path): self.path = path; self.duration = 3.0
            def close(self): pass

        with (
            patch.object(material.llm, "translate_terms",
                         return_value=["水质监测", "水处理"]),
            patch.object(material, "_download_files_from_kb",
                         return_value=kb_files),
            patch.object(material, "save_video", side_effect=fake_save),
            patch.object(material, "search_videos_pexels",
                         return_value=[pexels_item]) as mock_pexels,
            patch("app.services.material.VideoFileClip", FakeClip),
        ):
            result = material.download_videos(
                task_id="test-kb-fallback",
                search_terms=["water monitoring", "water treatment"],
                source="knowledge_base",
                audio_duration=22.0,
                max_clip_duration=3,
                kb_fallback_to_pexels=True,
            )

        self.assertTrue(any("video-a.mp4" in r for r in result))
        self.assertTrue(any("video-b.mp4" in r for r in result))
        self.assertTrue(any("pexels-supplement.mp4" in r for r in result),
                        "Pexels fallback NOT triggered")
        self.assertEqual(len(result), 3)
        self.assertTrue(mock_pexels.called,
                        "search_videos_pexels was NOT called")

    def test_kb_fallback_disabled_does_not_call_pexels(self):
        """
        kb_fallback_to_pexels=False 时，即使 KB 素材不足也不使用 Pexels。
        """
        config.app["material_directory"] = ""

        kb_files = ["/tmp/test-no-fallback/video-a.mp4"]
        ...
        with (
            ...
            patch.object(material, "_download_files_from_kb",
                         return_value=kb_files),
            patch.object(material, "search_videos_pexels") as mock_pexels,
            patch("app.services.material.VideoFileClip", FakeClip),
        ):
            result = material.download_videos(
                task_id="test-no-fallback",
                search_terms=["test"],
                source="knowledge_base",
                audio_duration=22.0,
    ...
    def test_match_script_order_estimate_triggers_pexels_fallback(self):
        """match_script_order=True 时，per_slot 估算使 Pexels 正确触发。"""
        config.app["pexels_api_keys"] = ["pexels-key"]
        config.app["material_directory"] = ""
        config.proxy.clear()
        N, K, D = 18, 5, 20.0
        _c = [0]
        def fsk(term, top_k=5, category=""):
            _c[0] += 1
            return [{"name": f"kb-video-{_c[0] % K}.mp4"}]
        pi = material.MaterialInfo(provider="pexels", url="https://x.com/p.mp4", duration=5)
        def fkd(n, d): return f"{d}/{n}"
        def fs(video_url, save_dir=""):
            return f"{save_dir}/saved.mp4"
        class FC:
            def __init__(s, p): s.p = p; s.duration = 3.0
            def close(s): pass
        with (
            patch.object(material.llm, "translate_terms", return_value=[f"w{i}" for i in range(N)]),
            patch.object(material, "_search_kb_with_fallback", side_effect=fsk),
            patch.object(material.kb_client, "download_media", side_effect=fkd),
            patch.object(material.kb_client, "is_healthy", return_value=True),
            patch.object(material, "save_video", side_effect=fs),
            patch.object(material, "search_videos_pexels", return_value=[pi]) as mp,
            patch("app.services.material.VideoFileClip", FC),
        ):
            r = material.download_videos(task_id="t", search_terms=[f"t{i}" for i in range(N)], source="knowledge_base", audio_duration=D, max_clip_duration=3, match_script_order=True, kb_fallback_to_pexels=True)
        self.assertTrue(mp.called, "Pexels not triggered: per_slot estimate should leave gap")
        kb = sum(1 for f in r if "kb-video" in str(f))
        self.assertEqual(kb, K, f"KB: {kb} != {K}")
