...
# DISABLED: incompatible with streamlit 1.59.1 -- from streamlit_tour import Tour
...
from app.services.kb_client import is_text_document, kb_client
...
# API 服务地址
_api_base = os.getenv("MPT_API_BASE", "http://127.0.0.1:8080")
...
def _task_video_url(video_file: str) -> str:
    """将服务器本地视频路径转换为浏览器可访问的 HTTP URL。"""
    import urllib.parse

    task_id = os.path.basename(os.path.dirname(video_file))
    filename = os.path.basename(video_file)
    parsed = urllib.parse.urlparse(_api_base)
    host = parsed.hostname or "127.0.0.1"
    port = parsed.port or 8080
    return f"{parsed.scheme}://{host}:{port}/tasks/{task_id}/{filename}"

def _dismiss_video_player_dialog():
    """关闭视频播放弹窗。"""
    st.session_state.pop("video_player_file", None)
    st.session_state.pop("video_player_subject", None)
...
def _get_active_running_task():
    """返回当前会话最近提交且仍在运行/暂停的任务 (task_id, state)。

    只依赖 state 存储中的真实状态；任务完成后（COMPLETE/FAILED）不再被选中，
    因此 active_generation_tasks 中残留的已完成任务不会误判为“可暂停”。
    """
    running = []
    for task_id, active_task in list(_active_generation_tasks().items()):
        try:
            ...
            task = None
        state = _normalize_task_state(task.get("state") if task else None)
        if state in (const.TASK_STATE_PROCESSING, const.TASK_STATE_PAUSED):
            running.append((active_task.get("mtime", 0), task_id, state))
    if not running:
        return None, None
    running.sort(key=lambda item: item[0], reverse=True)
    return running[0][1], running[0][2]
...
def _task_state_label(state, has_video):
    ...
    if normalized_state == const.TASK_STATE_PAUSED:
        return tr("Task Status Paused")
...
def _task_state_filter_key(task):
    ...
    if normalized_state in (const.TASK_STATE_PROCESSING, const.TASK_STATE_PAUSED):
        return "processing"
    if normalized_state == const.TASK_STATE_FAILED:
        return "failed"
...
def _collect_task_summaries(limit=20):
    ...
    for task in runtime_tasks:
        ...
        _script = task.get("script")
        _script_excerpt = (
            _script[:40] if isinstance(_script, str)
            else (_script[0][:40] if isinstance(_script, list) and _script else "")
        )
        subject = (
...
def _render_task_table(filtered_tasks, key_prefix):
    with st.container(key=f"task_table_header_{key_prefix}"):
        header_cols = st.columns([1.0, 1.5, 2.7, 0.8, 2.4], vertical_alignment="center")
    ...
    with st.container(height=list_height, border=False):
        for task in visible_tasks:
            ...
            is_paused = (
                _normalize_task_state(task["state"]) == const.TASK_STATE_PAUSED
            )
            ...
            with st.container(
                ...
                with action_cols[0]:
                    ...
                    if st.button(
                        ...
                        st.session_state["video_player_file"] = task["video_file"]
                        st.session_state["video_player_subject"] = task["subject"]
                        st.rerun(scope="app")

                with action_cols[1]:
                    download_label = tr("Download")
                    if has_video:
                        download_url = _task_video_url(task["video_file"])
                        st.markdown(
                            f'<a href="{download_url}" download="{os.path.basename(task["video_file"])}" '
                            f'style="text-decoration:none;" target="_blank">'
                            f'<button style="width:100%;padding:6px 12px;cursor:pointer;'
                            f'border:1px solid rgba(128,128,128,0.3);border-radius:0.5rem;'
                            f'background:rgba(128,128,128,0.1);color:inherit;font-size:0.9rem;">'
                            f'📥 {download_label}</button></a>',
                ...
                with action_cols[2]:
                    if is_paused:
                        st.button(
                            ...
                            use_container_width=True,
                            icon=":material/play_arrow:",
                            help=tr("Resume Generation"),
                            on_click=tm.resume_task,
                            args=(task_id,),
                        )
                    elif is_processing:
                        st.button(
                            tr("Pause"),
                            key=f"pause_task_{key_prefix}_{task_id}",
                            use_container_width=True,
                            icon=":material/pause:",
                            help=tr("Pause Generation"),
                            on_click=tm.pause_task,
                            args=(task_id,),
                        )
                    else:
                        st.button(
                            tr("Pause"),
                            key=f"pause_task_disabled_{key_prefix}_{task_id}",
                            use_container_width=True,
                            icon=":material/pause:",
                            disabled=True,
                        )
...
@st.dialog(
    tr("Video Player"),
    width="large",
    on_dismiss=_dismiss_video_player_dialog,
)
def _render_video_player_dialog(video_file: str, task_subject: str):
    """浏览器内嵌视频播放器弹窗。"""
    url = _task_video_url(video_file)
    filename = os.path.basename(video_file)
    st.caption(_format_task_subject(task_subject, max_length=80))
    st.video(url, format="video/mp4", subtitles=None)
...
def _render_generation_task_snapshot(task_id, task):
    ...
    if state == const.TASK_STATE_PAUSED:
        st.warning(tr("Video Generation Paused"))
        st.progress(
            progress,
            text=f"{tr('Task Progress')}: {progress}%",
        )
...
def sync_script_order_concat_mode():
    ...
    match_script_order = bool(st.session_state.get("match_materials_to_script", True))
...
def sync_material_driven_mode():
    """素材驱动模式开启时锁定素材来源为知识库并启用文案顺序匹配。"""
    if st.session_state.get("material_driven_mode"):
        st.session_state[localized_widget_key("video_source_select")] = "knowledge_base"
        st.session_state["match_materials_to_script"] = True
        sync_script_order_concat_mode()

def _render_category_cascade(tree, key_prefix: str = "md_cat"):
    """按一级/二级/三级…目录级联选择知识库分类，返回选中节点的 prefixes（逗号拼接）。

    每一级顶部提供「本级全部」选项，可停在任意中间层级聚合其子素材（例如选
    「血液透析水处理」→ 拿到开颜+AI机器人全部素材）；0 素材的子节点仍展示但
    置灰标注「暂无素材」，不可选，避免选到空分类；层级 selectbox 的 key 随父
    路径变化，父级切换时子级自动重置；返回 "" 表示无效。
    """
    _level_labels = ["一级目录", "二级目录", "三级目录", "四级目录", "五级目录"]
    _ALL = "__ALL__"
    path_names = []
    # 合成根：聚合所有一级分类，作为第一级的「本级全部」选项
    cur = {
        "count": sum(int(n.get("count", 0)) for n in tree),
        "prefixes": [p for n in tree for p in (n.get("prefixes") or [])],
        "children": tree,
    }
    while True:
        ...
        label = _level_labels[level] if level < len(_level_labels) else f"{level + 1}级目录"
        # 可选项：本级全部（有素材时）+ 有素材的子节点；0 素材子节点置灰展示
        options = []
        fmt = {}
        if int(cur.get("count", 0)) > 0:
            options.append(_ALL)
            fmt[_ALL] = f"本级全部（{int(cur.get('count', 0))} 个素材）"
        for n in children:
            name = n.get("name", "")
            if name and int(n.get("count", 0)) > 0:
                options.append(name)
                fmt[name] = f"{name}（{int(n.get('count', 0))} 个素材）"
        empty_names = [
            n.get("name", "")
            for n in children
            if n.get("name") and int(n.get("count", 0)) <= 0
        ]
        if not options:
            # 当前层级无任何可选节点（子节点全为 0 素材）
            return ""
        key = (
            (f"{key_prefix}_" + "_".join(path_names).replace("/", "_"))
            if path_names
            else f"{key_prefix}_l1"
        )
        choice = st.selectbox(
            label,
            options=options,
            key=key,
            format_func=lambda x, _f=fmt: _f.get(x, x),
        )
        if empty_names:
            st.caption("（暂无素材：" + "、".join(empty_names) + "）")
        if choice == _ALL:
            return ",".join(cur.get("prefixes") or [])
        child = next((n for n in children if n.get("name") == choice), None)
        if child is None:
            return ""
        path_names.append(choice)
        cur = child
    return ",".join(cur.get("prefixes") or [])
        ...
        with st.container(border=True):
            ...
            with st.container(key="advanced_settings_script"):
                with st.expander(tr("Advanced Script Settings"), expanded=False):
                    # --- 知识库集成 ---
                    _kb_available = kb_client.is_healthy()
                    if "use_knowledge" not in st.session_state:
                        st.session_state["use_knowledge"] = False
                    params.use_knowledge = st.checkbox(
                        tr("Use Knowledge Base for Script"),
                        key="use_knowledge",
                        help="Enable to generate scripts based on knowledge base content instead of LLM general knowledge. When enabled, video materials will also be sourced from the knowledge base.",
                    )

                    # KB 文档选择器（勾选「使用知识库」后显示）
                    if params.use_knowledge:
                        try:
                            _all_docs = kb_client.list_documents() if kb_client.is_healthy() else []
                        except Exception:
                            _all_docs = []

                        # 口播脚本只用文字文档：排除图片/视频（图片被向量化后也会混入 documents）
                        _all_docs = [d for d in _all_docs if is_text_document(d.get("filename", ""))]

                        if _all_docs:
                            # 层级分类筛选（只统计文字文档，与素材驱动脚本模式一致）
                            _doc_tree = []
                            try:
                                if kb_client.is_healthy():
                                    _doc_tree = kb_client.list_media_category_tree(file_type="doc")
                            except Exception:
                                _doc_tree = []
                            _doc_prefixes = ""
                            if _doc_tree:
                                _doc_prefixes = _render_category_cascade(_doc_tree, key_prefix="doc_cat")
                            _prefix_list = [p.strip() for p in _doc_prefixes.split(",") if p.strip()]

                            # 构建文档选项（按选中层级分类前缀过滤）
                            if _prefix_list:
                                _filtered = [
                                    d for d in _all_docs
                                    if any((d.get("category") or "").startswith(p) for p in _prefix_list)
                                ]
                            else:
                                _filtered = _all_docs

                            # 默认选中全部过滤后的文档
                            st.session_state.setdefault("kb_selected_docs", [])
                            _doc_options = {d["filename"]: f"{d['filename']} [{d.get('category', '')}]" for d in _filtered}

                            _selected = st.multiselect(
                                f"选择知识库文档（共 {len(_filtered)} 份，不选则自动检索）",
                                options=list(_doc_options.keys()),
                                format_func=lambda x: _doc_options.get(x, x),
                                default=st.session_state.get("kb_selected_docs", []),
                                key="kb_doc_multiselect",
                                help="勾选指定文档后，脚本和素材仅基于这些文档生成。留空则由系统自动检索。",
                            )
                            if _selected:
                                params.kb_doc_filenames = _selected
                            else:
                                params.kb_doc_filenames = None
                        else:
                            st.caption("知识库中暂无文档。")
                    ...
                    if not st.session_state.get("material_driven_mode"):
                        st.session_state.setdefault("video_script_duration_input", 0)
                        params.video_script_duration = int(
                            st.number_input(
                                "脚本时长（秒）",
                                min_value=0,
                                max_value=300,
                                step=5,
                                value=0,
                                key="video_script_duration_input",
                                help="目标口播时长，0 = 不限制。非 0 时按 4.2 字/秒换算目标字数，"
                                "生成后自动核对并在超差时重写（±15%）。",
                            )
                        )
            ...
            if st.button(
                ...
                _material_driven = bool(
                    st.session_state.get("material_driven_mode")
                    and st.session_state.get("selected_category")
                )
                if _material_driven:
                    # 素材驱动：基于选中分类下素材的文件名 + 视觉描述生成脚本
                    with st.spinner(tr("Generating Video Script and Keywords")):
                        _md_cat = st.session_state.get("selected_category")
                        _kb_media = kb_client.list_media_sampled(_md_cat)
                        # 素材驱动模式下脚本时长控件渲染在页面后部（按钮之后），
                        # 故从 session_state 读取，避免 params 尚未赋值。
                        _md_duration = int(
                            st.session_state.get("md_script_duration_input", 0) or 0
                        )
                        _sb = llm.generate_script_from_materials(
                            kb_media=_kb_media,
                            video_subject=params.video_subject,
                            language=params.video_language,
                            paragraph_number=params.paragraph_number,
                    ...
                    if _sb:
                        ...
                        st.session_state.pop("video_terms_cn", None)
                    else:
                        st.error("素材驱动脚本生成失败，请重试")
                    ...
                    with st.spinner(tr("Generating Video Script and Keywords")):
                        with config.runtime_config_lock():
                            ...
                            script, kb_info = script_result if isinstance(script_result, tuple) else (script_result, {})
                            _video_source = getattr(params, 'video_source', 'pexels') or 'pexels'
                        ...
                        if "Error: " in script:
                            ...
                            # For Pexels/Pixabay sources: translate English terms to Chinese for display
                            if _video_source not in ("knowledge_base", "local"):
                                _cn_terms = llm.translate_terms(terms)
                                if _cn_terms and any(_cn_terms):
                                    st.session_state["video_terms_cn"] = _cn_terms
                                else:
                                    st.session_state.pop("video_terms_cn", None)
                            else:
                                st.session_state.pop("video_terms_cn", None)
            _pending_script = st.session_state.pop("video_script_pending", None)
            if _pending_script is not None:
                st.session_state["video_script"] = _pending_script
            ...
            # Show real-time estimated voiceover duration below the script textarea
            _script_text = str(params.video_script or "").strip()
            if _script_text:
                _est = _estimate_voiceover_duration_range(
                    _script_text,
                    params.voice_rate,
                )
                if _est:
            ...
            # Show Chinese translation annotations for Pexels/Pixabay mode
            if st.session_state.get("video_terms_cn"):
                _cn_lines = []
                _en_terms = [t.strip() for t in st.session_state.get("video_terms", "").split(",") if t.strip()]
                for _i, _cn in enumerate(st.session_state["video_terms_cn"]):
                    if _cn and _i < len(_en_terms):
                        _cn_lines.append(f"{_en_terms[_i]} → {_cn}")
                if _cn_lines:
                    st.caption("中文注释：" + " | ".join(_cn_lines))
    ...
    with panel:
        with st.container(border=True):
            ...
            video_sources = [
                ...
                (tr("Coverr"), "coverr"),
                (tr("Local file"), "local"),
                (tr("Knowledge Base"), "knowledge_base"),
                (tr("Seedance"), "jimeng"),
            ]
            ...
            if params.video_source in ("knowledge_base", "jimeng"):
                st.session_state.setdefault("kb_category", "")
                # 即梦 Seedance 只取图片；知识库源用全部媒体素材
                _ft = "image" if params.video_source == "jimeng" else "all"
                _kb_tree = []
                try:
                    if kb_client.is_healthy():
                        _kb_tree = kb_client.list_media_category_tree(file_type=_ft)
                except Exception:
                    _kb_tree = []
                if _kb_tree:
                    params.kb_category = _render_category_cascade(_kb_tree, key_prefix="kb_cat")
                else:
                    params.kb_category = ""
                    st.warning("知识库暂无分类素材，请先上传素材。")

            # ── 即梦 Seedance 宣传片模式 ──
            if params.video_source == "jimeng":
                st.markdown("---")
                st.subheader("🎬 即梦 Seedance 宣传片")

                if st.button(
                    "🎬 生成宣传片分镜脚本",
                    key="generate_seedance_storyboard",
                    use_container_width=True,
                    type="secondary",
                ):
                    if not (params.video_subject or "").strip():
                        st.warning("请先输入视频主题")
                    elif not (getattr(params, "kb_category", "") or ""):
                        st.warning("请先选择知识库层级分类")
                    else:
                        with st.spinner("Kimi 识图 + DeepSeek 创作中（约需 1-2 分钟）..."):
                            try:
                                _preview_dir = tempfile.mkdtemp(prefix="seedance_preview_")
                                _shots = generate_promo_storyboard(
                                    kb_category=params.kb_category,
                                    task_dir=_preview_dir,
                                    video_subject=params.video_subject,
                                )
                                if not _shots:
                                    st.error("分镜生成失败：请检查知识库系列图片与 moonshot_api_key")
                                else:
                                    _script, _shots = llm.generate_jimeng_storyboard_and_script(
                                        video_subject=params.video_subject,
                                        shots=_shots,
                                        language=params.video_language,
                                    )
                                    st.session_state["video_script_pending"] = _script
                                    st.session_state["seedance_storyboard"] = _shots
                                    # 递增版本号，让画面提示词 textarea 的 key 变化，重新生成后刷新显示
                                    st.session_state["seedance_storyboard_ver"] = (
                                        st.session_state.get("seedance_storyboard_ver", 0) + 1
                                    )
                                    st.rerun()
                            except Exception as _e:
                                st.error(f"生成失败：{_e}")

                _preview_shots = st.session_state.get("seedance_storyboard")
                if _preview_shots:
                    st.markdown("#### 📋 宣传片画面提示词")
                    st.caption("每个镜头的画面提示词可直接编辑，提交后即梦按编辑后的提示词生成视频。")
                    _ver = st.session_state.get("seedance_storyboard_ver", 0)
                    for _i, _s in enumerate(_preview_shots):
                        _idx = _s.get("index", _i + 1)
                        _media = _s.get("media", "")
                        _cur = _s.get("camera_prompt") or _s.get("visual_description", "")
                        _new = st.text_area(
                            f"镜头 {_idx} · {_media}",
                            value=_cur,
                            height=90,
                            key=f"seedance_camera_prompt_v{_ver}_{_idx}",
                        )
                        _s["camera_prompt"] = _new.strip()
                    st.session_state["seedance_storyboard"] = _preview_shots
            ...
            selected_concat_mode = stable_selectbox(
                ...
                disabled=bool(st.session_state.get("match_materials_to_script", True)),
            )
            ...
            # ── 素材驱动脚本模式 ──
            params.material_driven_mode = st.checkbox(
                "素材驱动脚本模式",
                key="material_driven_mode",
                on_change=sync_material_driven_mode,
                help="勾选后从知识库勾选素材，基于素材文件名与视觉描述生成脚本；生成视频时在勾选素材内重新匹配并裁剪多余素材。",
            )

            if params.material_driven_mode:
                _tree = []
                try:
                    if kb_client.is_healthy():
                        _tree = kb_client.list_media_category_tree()
                except Exception:
                    _tree = []
                if _tree:
                    _selected_cat = _render_category_cascade(_tree)
                    st.session_state["selected_category"] = _selected_cat
                    params.selected_category = _selected_cat or None
                else:
                    st.warning("知识库暂无分类素材，请先上传素材。")
                    params.selected_category = None

                # 脚本时长（与普通模式一致的时长约束，素材驱动模式下在此处设置）
                st.session_state.setdefault(
                    "md_script_duration_input",
                    int(st.session_state.get("video_script_duration_input", 0) or 0),
                )
                params.video_script_duration = int(
                    st.number_input(
                        "脚本时长（秒）",
...
def _estimate_voiceover_duration_range(
    ...
    # voice_rate 只作为估算修正项。底层口径统一委托 utils.estimate_voiceover_duration，
    # 此处仅保留 ±15% 展示区间，避免让用户误以为该值等同于服务端真实结果。
    estimated_seconds = utils.estimate_voiceover_duration(
        normalized_text, voice_rate=voice_rate
    )
...
def _render_generation_controls(
    ...
    active_task_id, active_task_state = _get_active_running_task()
    ...
    elif active_task_state == const.TASK_STATE_PAUSED:
        # 任务已暂停：按钮切换为“继续生成”，点击后恢复流水线。
        start_button = False
        st.button(
            tr("Resume Generation"),
            use_container_width=True,
            type="primary",
            key="resume_generation_button",
            icon=":material/play_arrow:",
            on_click=tm.resume_task,
            args=(active_task_id,),
        )
    else:
        # 任务运行中：按钮切换为“暂停生成”，点击后在阶段边界暂停。
        start_button = False
        st.button(
            tr("Pause Generation"),
            use_container_width=True,
            type="primary",
            key="pause_generation_button",
            icon=":material/pause:",
            on_click=tm.pause_task,
            args=(active_task_id,),
        )
#     render_onboarding_tour()
    if start_button:
        ...
        if params.video_source not in ["pexels", "pixabay", "coverr", "local", "knowledge_base", "jimeng"]:
            _remove_active_generation_task(task_id)
        ...
        # ── 即梦模式前置检查 ──
        if params.video_source == "jimeng":
            if not config.app.get("moonshot_api_key", ""):
                _remove_active_generation_task(task_id)
                st.error("即梦模式需要 Kimi 视觉模型，请在 config.toml 配置 moonshot_api_key")
                st.stop()
            if not config.app.get("volcengine_api_key", ""):
                _remove_active_generation_task(task_id)
                st.error("即梦模式需要即梦视频生成，请在 config.toml 配置 volcengine_api_key")
                st.stop()
            if not getattr(params, "kb_category", ""):
                _remove_active_generation_task(task_id)
                st.error("即梦模式需要先选择知识库层级分类")
                st.stop()
        ...
        # 即梦模式：把前端预览并编辑过的分镜（含画面提示词）序列化传给后端复用
        if params.video_source == "jimeng" and st.session_state.get("seedance_storyboard"):
            params.jimeng_storyboard = json.dumps(
                st.session_state["seedance_storyboard"], ensure_ascii=False
            )
...
def _render_application():
    ...
    video_player_file = st.session_state.get("video_player_file")
    if video_player_file:
        _render_video_player_dialog(
            video_player_file,
            st.session_state.get("video_player_subject", ""),
        )
    ...
    params.match_materials_to_script = bool(
        st.session_state.get("match_materials_to_script", True)
    )
