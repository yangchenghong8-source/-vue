...
class VideoParams(BaseModel):
    ...
    video_subject: str = Field(default="", description="Video subject/topic")

    @field_validator("video_subject")
    @classmethod
    def validate_subject_strip(cls, v: str) -> str:
        """Strip whitespace; empty is allowed (rejected at API boundary)."""
    ...
    # ── 知识库集成 ──
    use_knowledge: bool = False  # 是否使用知识库内容生成脚本
    kb_doc_filenames: Optional[List[str]] = None  # 指定使用的知识库文档（空=自动检索）
    kb_category: Optional[str] = ""  # KB 产品线分类，空=不过滤
    # 即梦模式：前端预览并编辑过的分镜（JSON 数组字符串，含 index/media/camera_prompt/
    # visual_description/duration/clip_duration），后端按 media 重新下载图并复用画面提示词，
    # 避免重复取图/识图/DeepSeek 创作。空串 = 未预览，走正常生成链路。
    jimeng_storyboard: Optional[str] = ""
    # ── 素材驱动脚本模式 ──
    material_driven_mode: bool = False  # 开关：选分类→生成脚本→重配素材
    ...
    font_name: Optional[str] = config.ui.get("font_name", "MicrosoftYaHeiBold.ttc")
    ...
    # 目标口播时长（秒）。0 = 不限制，走原有生成行为；非 0 时按 4.2 字/秒
    # 换算成目标字数写进 prompt，并在 generate_script 内闭环重写达标准。
