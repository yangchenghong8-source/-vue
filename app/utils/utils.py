...
def estimate_voiceover_duration(text, voice_rate=1.0):
    """估算口播文案时长（秒），返回 None 表示无可估算内容。

    与前端 `_estimate_voiceover_duration_range` 共用同一套语速口径：
    中文/日文/韩文按 4.2 字/秒，其它按 2.6 词/秒，标点按 0.12 秒停顿，
    voice_rate 作为倍率修正项。该估算用于脚本生成闭环的时长核对，
    不参与真实 TTS 合成，因此返回单点值（非区间）即可。
    """
    ...
    return base_seconds / normalized_rate
