...
@router.post("/voice-preview", summary="Generate a short TTS voice preview")
def preview_voice(body: dict = Body(...), user: User = Depends(_get_current_user)):
    voice_name = str(body.get("voice_name") or "").strip()
    text = str(body.get("text") or "").strip()
    if not voice_name or voice_service.is_no_voice(voice_name):
        raise HttpException(task_id="voice-preview", status_code=400, message="请选择有效音色")
    if not text:
        raise HttpException(task_id="voice-preview", status_code=400, message="试听文本不能为空")
    voice_rate = float(body.get("voice_rate") or 1.0)
    voice_volume = float(body.get("voice_volume") or 1.0)
    fd, output_path = tempfile.mkstemp(prefix=".voice-preview-", suffix=".mp3")
    os.close(fd)
    try:
        result = voice_service.tts(text[:240], voice_name, voice_rate, output_path, voice_volume)
        if result is None or not os.path.isfile(output_path) or os.path.getsize(output_path) == 0:
            raise RuntimeError("TTS returned no audio")
    except Exception as exc:
        try: os.remove(output_path)
        except OSError: pass
        raise HttpException(task_id="voice-preview", status_code=502, message=f"试听生成失败：{exc}") from exc
    return FileResponse(output_path, media_type="audio/mpeg", filename="voice-preview.mp3", background=BackgroundTask(lambda: os.remove(output_path) if os.path.exists(output_path) else None))
