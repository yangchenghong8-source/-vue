...
# ── Secret overrides: env vars take precedence over config.toml ─────────
# This keeps API keys out of version control while preserving the config.toml
# workflow for local development.  Set the corresponding environment variable
# in production (systemd unit, docker-compose, or .env file).
...
for _key in _SECRET_KEYS:
    _env_val = os.getenv(f"MPT_{_key.upper()}")
    if _env_val:
        app[_key] = _env_val

# Per-service secrets that use different config sections
if os.getenv("MPT_SILICONFLOW_API_KEY"):
    siliconflow["api_key"] = os.getenv("MPT_SILICONFLOW_API_KEY")

if os.getenv("MPT_ELEVENLABS_API_KEY"):
    elevenlabs["api_key"] = os.getenv("MPT_ELEVENLABS_API_KEY")

if os.getenv("MPT_AZURE_SPEECH_KEY"):
    azure["speech_key"] = os.getenv("MPT_AZURE_SPEECH_KEY")

if os.getenv("MPT_PEXELS_API_KEYS"):
    app["pexels_api_keys"] = [
        k.strip() for k in os.getenv("MPT_PEXELS_API_KEYS").split(",") if k.strip()
    ]

if os.getenv("MPT_PIXABAY_API_KEYS"):
    app["pixabay_api_keys"] = [
        k.strip() for k in os.getenv("MPT_PIXABAY_API_KEYS").split(",") if k.strip()
    ]

# API access control — used by the auth middleware (section 2.5)
app["api_auth_key"] = os.getenv("MPT_API_AUTH_KEY", app.get("api_auth_key", ""))
app["webui_password"] = os.getenv("MPT_WEBUI_PASSWORD", app.get("webui_password", ""))
